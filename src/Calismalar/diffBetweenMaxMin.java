...
public class diffBetweenMaxMin {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("Lütfen liste elemanlarını aralarında boşluk bırakarak giriniz : ");
    ...
    // Kodlamaya burdan başla.Bu satırdan önceki satırlarda hiçbirşeyi kaldırma ve degiştirme. Kullanacağın array 27.ci satırda  --> useThisArray
        ...
        System.out.println("Max : "+maxBul(useThisArray));
        System.out.println("Min : "+minBul(useThisArray));
    }
    ...
    private static int maxBul(int[] useThisArray) {
        int max=0;
        for (int i = 0; i < useThisArray.length; i++) {
            max=Math.max(max, useThisArray[i]);             // [7, 2, 10, 9]
        }
        return max;
    }

    private static int minBul(int[] useThisArray) {
